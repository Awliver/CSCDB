...
from tpcc_batch import build_prepare_stmts, install_prepare, run_txn_batch
...
class TxnStats:
    ...
    def record(self, txn_type, ok, err="", latency=0.0, batch_latencies=None, warehouse=None):
        with self.lock:
...
def worker_loop(
    ...
    try:
        ...
        while time.perf_counter() < end and not stop_event.is_set():
            ...
            try:
                if use_batch:
                    ok, err = run_txn_batch(cli, rng, worker_scale, txn_name, route=route)
                ...
                ok, err = False, "worker exception %s: %s" % (type(e).__name__, e)
            dt = time.perf_counter() - t0
            stats.record(
                txn_name, ok, err, dt, batch_latencies,
                warehouse=worker_scale.get("w_id"),
            )
    finally:
        cli.close()
...
def bench_round(
    ...
    if no_fail or o_fail:
        print("    last error:", snapshot["last_err"][:160])
    ...
    snapshot.update({"tpm": tpm, "elapsed": elapsed})
    return snapshot
...
def main():
    ...
    ap.add_argument("--reuse-db", action="store_true",
                    help="reuse build/tpcc_perf_db in place (diagnostic runs only)")
    ...
    if args.strict:
        if (args.skip_crash or args.skip_consistency or args.skip_p2 or
                args.skip_load_content or args.skip_load_counts or args.reuse_db):
    ...
    if args.scale == "full" and needs_full_csv:
