...
def start_rmdb(db_name, log_path=None, template_dir=None, reuse_existing=False):
    ...
    if reuse_existing and not os.path.isfile(os.path.join(dbpath, "db.meta")):
        raise RuntimeError("cannot reuse missing database: " + dbpath)
    if os.path.isdir(dbpath) and not reuse_existing:
