...
path = os.path.join(BUILD, "phantom_dbg")
if os.path.exists(path):
    shutil.rmtree(path)
proc = subprocess.Popen(["./bin/rmdb", "phantom_dbg"], cwd=BUILD, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
time.sleep(2)
...
for sess, sql in schedule:
    out = send(s[sess], sql)
    print(f"S{sess} {sql!r} -> {out.strip()!r}")
proc.terminate()
