...
Faithful points vs the team's old harness:
  ...
  - StockLevel: random threshold 10..20, single-statement native COUNT(DISTINCT (col))
  - Real timestamps
...
def stock_level(cli, r, cfg, w_id, d_id=None):
    ...
    # 决赛规范：引擎原生支持 COUNT(DISTINCT ...)，StockLevel 须只发单条查询，
    # 禁止客户端对 join 结果去重。
    rows, err = _select(
        cli, "select count(distinct (s_i_id)) from order_line, stock where ol_w_id = %d and ol_d_id = %d "
             "and ol_o_id >= %d and ol_o_id <= %d and s_w_id = %d and s_i_id = ol_i_id "
             "and s_quantity < %d;" % (w_id, d_id, lo, hi, w_id, threshold), "stock join")
    if err:
        return err
    _ = one_int(rows, 0) if rows else 0      # low-stock distinct item count
