...
# 分步
python3 tests/bench/tpccbench.py gen   --scale full -W 1          # 生成规范数据 (W=1 全量)
python3 tests/bench/tpccbench.py load  --scale full                # 建库+装载+建索引+行数校验
python3 tests/bench/tpccbench.py smoke                             # 每种事务的精确副作用断言
python3 tests/bench/tpccbench.py check --samples 16 --deep         # 一致性条件 C1..C12
python3 tests/bench/tpccbench.py acid                              # 原子/隔离/持久性(kill -9)
python3 tests/bench/tpccbench.py run --duration 300 --clients 8 --json out.json
```
