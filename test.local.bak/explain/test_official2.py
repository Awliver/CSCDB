...
# Start server with stderr to a file
stderr_file = open(os.path.join(BUILD_DIR, "server_stderr.txt"), "w")
...
if not wait_for_port(PORT, timeout=6):
    ...
    stderr_file.flush()
    with open(os.path.join(BUILD_DIR, "server_stderr.txt"), "r") as f:
...
# First test simpler query
q = "SELECT * FROM customers c JOIN orders o ON c.customer_id = o.customer_id WHERE o.total_amount > 1000;"
print(f"\n=== Query: {q} ===")
result = send_sql(sock, q)
print(repr(result))

q2 = "EXPLAIN ANALYZE SELECT * FROM customers c JOIN orders o ON c.customer_id = o.customer_id WHERE o.total_amount > 1000;"
print(f"\n=== Query: {q2} ===")
result2 = send_sql(sock, q2)
print(repr(result2))
