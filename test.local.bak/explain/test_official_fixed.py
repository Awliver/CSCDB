...
DB_NAME = "test_official_db_fixed"
...
queries = [
    ("EXPLAIN ANALYZE SELECT * FROM customers, orders WHERE customers.customer_id = orders.customer_id AND orders.total_amount > 1000;",
     """Project(columns=[customers.address, customers.customer_id, customers.email, customers.name, orders.customer_id, orders.order_date, orders.order_id, orders.total_amount], rows=2)
\tJoin(tables=[customers, orders],condition=[customers.customer_id=orders.customer_id], rows=2)
\t\tFilter(condition=[orders.total_amount>1000.0], rows=2)
\t\t\tScan(table=orders, type=SeqScan, rows=5)
\t\tScan(table=customers, type=SeqScan, rows=3)
"""),
    ("EXPLAIN ANALYZE SELECT customers.name, orders.order_id FROM customers, orders WHERE customers.customer_id = orders.customer_id;",
