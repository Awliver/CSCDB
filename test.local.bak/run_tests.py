...
def start_server(db_name):
    ...
    # 在 build/ 目录下启动服务器；argv 用相对路径 test_dbs/<name>
    ...
    for attempt in range(3):
        ...
        try:
            ...
            last_err = err.decode("utf-8", errors="replace") if err else ""
        except subprocess.TimeoutExpired:
            proc.kill()
            proc.wait()
            last_err = "communicate timeout"
        else:
            last_err = last_err or "port not listening"
        kill_existing_server()
        time.sleep(0.5 * (attempt + 1))
    raise RuntimeError("服务器启动失败或端口未监听" + (f": {last_err}" if last_err else ""))
...
def read_output(db_name):
    path = os.path.join(DB_BASE_DIR, db_name, "output.txt")
...
def run_testpoint(name, db_name, sqls, expected, unordered_steps=None):
    ...
    actual = read_output(db_name)
    out_path = os.path.join(DB_BASE_DIR, db_name, "output.txt")
    print(f"\n----- output.txt ({out_path}) -----")
    print(actual, end="")
    print("----- end -----")
    ...
    if unordered_steps:
        # 按步骤拆分 output.txt
        # 假设每个 select 之间由非表头行分隔，这里简单按步骤分组
        # 实际实现：找到每个 select 的结果块，对指定步骤做集合比较
        actual_blocks = []
        block = []
