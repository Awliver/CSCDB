...
commands = [
    "CREATE TABLE A (a_id int, x int, y int);",
    "CREATE TABLE B (b_id int, x int, z int);",
    "CREATE TABLE C (c_id int, y int);",
...
queries = [
    "EXPLAIN ANALYZE SELECT A.a_id, B.b_id, C.c_id FROM A, B, C WHERE A.x = B.x AND A.y = C.y AND B.z > 150;",
    "SELECT A.a_id, B.b_id, C.c_id FROM A, B, C WHERE A.x = B.x AND A.y = C.y AND B.z > 150;",
]
