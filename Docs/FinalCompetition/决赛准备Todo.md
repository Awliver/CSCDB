...
> **线下赛与答辩**：[`线下赛与答辩准备计划`](./线下赛与答辩准备计划.md)（8/16 报到、8/17 无 AI 功能题、8/18 多轮答辩、8/19 颁奖）
...
| 性能只做可控增量 | 以 `c446377` 官方 PASS 为基线；适配 **W=50×32**；禁 `sleep_for` 类高风险；一次只交一个变量 |
| 排名优先 | 所有优化先满足比赛规则、项目规范、Wire 语义与 ACID；此前提下唯一目标是尽可能提高三窗中位 **NewOrder/min（Rank）**。abort-rate 仅作健康约束/诊断，不能本末倒置为优化终点 |
| 合规 | 禁止任何表名/列名/SQL/stmt ID/固定 ID/种子/数据分布/阶段的硬编码或旁路；ACID 不得为性能降级；禁改 `CMakeLists.txt` |
...
### 0.1 08-05 OJ 样本：低 abort 不是高排名的充分条件

| 项 | 08-05 12:24 样本 | 含义 |
|------|------:|------|
| 结果 | 全流程 PASS | 正确性、恢复、持久化、装载、三窗与崩后检查均通过 |
| Ranking | **29,650.4 NewOrder/min** | 低于 `c446377` 的 40,264.8；不替换性能锚点 |
| abort-rate | **0.17%** | 指标极低，但不是排名分子 |
| NewOrder | 229,593 attempted / 227,241 committed / 5 abandoned | NewOrder 的意外放弃已近乎为零，继续压低它没有可观收益 |
| 延迟 / 利用率 | p50 19.479ms，p99 192.201ms；CPU 平均 2.56/16 核 | 更符合准入排队、同步 I/O 或其他等待限制，而非 CPU 饱和 |

该样本禁止被解读为“应继续降低 abort-rate”。在固定测量窗口内，过宽的热点准入或更长的等待会把本可快速 abort/retry 的冲突变成队列等待，压低事务发起速率和并行度；即使 abandoned 降低，Rank 仍会下降。所有性能优化均须先合规、再以尽可能提高 Rank 为唯一目标；选择并发控制策略时：

1. 先确保全部门禁、NewOrder 可持续提交、覆盖与恢复正确；
2. 分事务族记录 NewOrder 的 unexpected abandon、业务预期回滚和非排名事务的 abandon，禁止只看总 abort-rate；
3. 对每个候选在相同 W=50×32、3×150s 口径比较 Rank 中位数、p99、5s CV/最低桶、准入等待与 I/O；
...
| **08-03 08:58** | **P-A1 提交评测 `c446377`** | **PASS**；25 功能 + 5 恢复、COMMIT 32/32、三窗 50/50、崩溃恢复及 I/O 诊断全过；装载 **620.61s**；三轮 38,846 / 40,264.8 / 41,428，中位 **40,264.8**，较旧锚点 +8.10% 并升级为新锚点 |

> 当前 `full` 表示 W=50，W=5 趋势须显式使用 `--scale local`。P-A1 的 W=5 五样本/侧单变量口径、原始路径与噪声警告见 [`Optimize/15`](../Optimize/15.P-A1归因取证.md)。
...
| 3 | H1/H2 索引边角 | ⬜ | 一致性 §三 |
| 4 | 装载 900s 预算 + 动态 `order_line` | ✅ | 最新官方：W=50、9 表 25,051,681 行，**620.61s**；`order_line` 已通过全量装载与恢复检查 |
...
> 08-03 `c446377` 官方全流程已通过；本机 W=50 开启态终验成本不可接受，P-A1 与后续性能实验均冻结。裁决 → [`Optimize/14`](../Optimize/14.决赛优化状态与实施计划.md)；证据 → [`Optimize/15`](../Optimize/15.P-A1归因取证.md)。
...
| 1 | P-A1 两次 W=50 开启态归因终验 | 🧊 | 实现/PA1/W=5 已完成；本机成本不可接受，终验未执行且不作为交付门禁 |
| 2 | district/stock 冲突窗口收缩 | 🧊 | 未取得 W=50 第一原因；禁止按 W=5 结果启动 |
| 3 | PREPARE_SET 分析/元数据缓存 | 🧊 | 未取得 W=50 解析/计划热点证据 |
| 4 | P3 字节/等待者阈值组提交 | 🧊 | 未取得 W=50 WAL 触发证据；默认关闭 |
...
| 6 | 总体架构定稿、文档归档与线下答辩准备 | 🟠 | 见 [`线下赛与答辩准备计划`](./线下赛与答辩准备计划.md)；8/17 功能题优先级最高，现场禁止 AI |
...
- [x] 装载正式数据在 900s 内完成（最新官方：W=50、**620.61s**；25,051,681 行）
- [x] （排名）W=50×32 / SI / 3×150s 可跑通且仓库覆盖门禁过（最新 `c446377`：3/3 窗、32 客户端、50/50 覆盖）
- [x] P-A1 稳定 token、failed_op、AUTO_ABORT、5s 统计与四分类对账确定性用例 PASS；W=5 五样本/侧开关中位 -0.041%
- [x] P-A1 W=50 开启态终验的项目冻结决策：本机成本不可接受，未执行、不记为通过或失败，也不作为交付门禁
- [x] 无表名/SQL 硬编码旁路；未改 CMake；ACID 未降级
- [ ] 架构图、P1–P10 映射、答辩证据表与 PPT 口径一致
- [ ] 完成无 AI 的限时功能题演练、主讲彩排与专家追问彩排
- [ ] 离线构建、材料备份、演示命令与现场规则清单齐全
...
| **2026-08-03** | **P-A1 与新锚点 / 项目冻结**：`c446377` 官方全流程 PASS；W=50 装载 620.61s，三轮 38,846 / 40,264.8 / 41,428，中位 **40,264.8**，较旧锚点 +8.10%。本地 PA1 用例 PASS；W=5 五样本/侧统计开关中位 **-0.041%**，ON×5 原因 Top-3 行覆盖 **80.11%**、主原因占比极差 **4.00pp**。因本机 W=50 成本不可接受，P-A1 开启态终验及 P-A2/P-A3/P-W1 冻结，转入架构定稿与答辩准备。 |
| **2026-08-03** | **线下赛与答辩计划**：明确 8/16 报到、8/17 线下功能题、8/18 多轮答辩、8/19 颁奖的倒排计划；功能题未完成会取消成绩，因此无 AI 限时演练、离线构建与定向测试优先于 PPT 和性能工作。 |
