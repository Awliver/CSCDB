...
> **⚡ 状态速览（2026-07-30 晚）**：OJ **全 PASS**，首个有效成绩 **10402 NewOrder/min**。
> 正确性门禁全部通过；当日六份报告的根因链与修复见 [`0730-OJ战役复盘`](./0730-OJ战役复盘.md)。
> 当前主线转入**排名优化**：① 第 3 轮塌陷（11110.8/10402/7324.8，轮内积压形态）；
> ② Delivery 尾延迟（28.7% 放弃、p50 777ms）——两者大概率同源（new_orders 墓碑/链积压）。
> 本文以下内容为 07-27 版规划，门禁部分已完成，保留作检查清单。

> **末次修订**：2026-07-27（初赛结束 → 切入决赛主线）  
...
> **一致性明细**：[`一致性审计与回归用例`](./一致性审计与回归用例.md) · **性能候选**：[`Optimize/2`](../Optimize/2.待落地优化Todo.md)
...
| 性能只做可控增量 | 门禁通过后再动；适配 **W=50×32**；禁 `sleep_for` 类高风险；一次只交一个变量 |
...
| 3 | **SSI 语句级即时中止** | 🟡 | 主路径（SeqScan/IndexScan/Update/Insert/Delete）已在**语句执行中**即时 `TRANSACTION_ABORT`，非拖到 COMMIT；残留缺口见 §2 P1-1（INLJ 无 SSI 读跟踪） |
...
> 上表按 2026-07-27 代码现状核对（非仅 Todo 计划状态）；🟡 = 主路径已实现但有已知残留缺口（详见 §2 P1）。
...
| 序 | 项 | 状态 | 来源 |
|:--:|----|:----:|------|
| 1 | H3 INLJ 缺 SSI · H4 `ser_finish` 无锁读 | ⬜ | 一致性 §三 |
| 2 | C3 Cleaner TOCTOU · C4 脏页/日志序 · C5 `page_lsn` | ⬜ | 一致性 §三 |
| 3 | H1/H2 索引边角 | ⬜ | 一致性 §三 |
| 4 | 装载 900s 预算 + 动态 `order_line` | ⬜ | 决赛规格；大表 LOAD/索引/内存 |
...
## 3. P2 — 性能（门禁通过后 · W=50×32）

> 仅在 §1.2 门禁项达标后启动。细则 → [`Optimize/2`](../Optimize/2.待落地优化Todo.md)  
> 正式规格：**50 仓 × 32 客户端** · 预热 30s · **3×150s** · 隔离 **SI**  
> 本地 W=5 脚本仅作趋势，**不能替代**决赛自检。

| 序 | 项 | 状态 | 备注 |
|:--:|----|:----:|------|
| 1 | 评估上线 **`2ec06f0` P2 UPDATE_DELTA** | 🧊 | 单独交；须 consistency + 决赛规格复测 |
| 2 | 评估上线 **`31e5fac` SER 谓词预编译** | 🧊 | 与 P2 **分项**；功能 SER 路径 |
| 3 | T4 `update_district` 尾延迟 | 🧊 | 热点仓 / 热点 district |
| 4 | T1 字节/等待者频率组提交 | 🧊 | 须满足可审计 fsync 契约；**禁止默认 sleep_for** |
| 5 | T3 限核/cgroup 复测 | 🧊 | S5.2 教训 |
| 6 | 本地 TPC-C 走 OJ BATCH 热路径 | ✅ | `tpcc_batch.py` + 默认 `PREPARE_SET+EXEC_BATCH`；批次边界对齐附件 A §7；`--stream` 仅 A/B |
...
| 2 | 决赛形本地 harness（Wire BATCH / SI / 150s×32 / 混合比） | 🟡 | **协议+窗口+混合比已齐**（`--finals`）；缺 W=50 正式 CSV / 仓库覆盖门禁 |
| 3 | 对照 PDF「参赛提交前自检清单」逐项勾选 | ⬜ |
| 4 | 维护 Optimize §5 / §9 与本 Todo 状态 | ⬜ |
| 5 | M1–M7 等 Medium 项按需排期 | ⬜ |
...
- [x] SI / SER 可配置；SSI 语句级即时中止（主路径；INLJ 残留缺口见 §2 P1-1）
...
- [ ] 装载正式数据在 900s 内完成（W=50 待实测）
- [ ] （排名）W=50×32 / SI / 3×150s 可跑通且仓库覆盖门禁过（`--finals` 已可跑 W=5；待 W=50 数据）
