...
| 3 | [08-02 OJ 报告摘要](./FinalCompetition/0802-OJ性能测评报告摘要.md) | **最新 `5ae93e4` 全 PASS · 33,862.8；H3/H4 已冻结，性能锚点 37,248.4** |
...
| — | [2.Todo](./Optimize/2.待落地优化Todo.md) | 决赛活动队列（H3/H4 已冻结；归因 → 条件式性能实验） |
...
│   ├── 决赛准备Todo.md          ← 决赛主线
│   ├── 0802-OJ性能测评报告摘要.md ← 最新 H4 冻结 PASS
...
| 本地 / gitlab | HEAD `5ae93e4`（H4 冻结；含 H3 `d39fcdb`） |
| 本机远端 | 仅 **`gitlab`** |
| **阶段** | **决赛**：正确性门禁 → Wire v3 / SI·SER / COUNT(DISTINCT) / FLOAT32 → W=50×32 |
| **最新官方评测** | `5ae93e4` **PASS**；**33,862.8 NewOrder/min**；装载 **700.54s**；性能锚点仍为 `c524ff8` **37,248.4** |
| 正确性状态 | H3 INLJ SSI、H4 `active_rts_` 锁域均已冻结；下一步为 abort 归因 |
